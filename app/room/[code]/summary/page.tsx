...
function SummaryContent({ roomCode }: { roomCode: string }) {
  ...
  const [roomDurationMinutes, setRoomDurationMinutes] = useState<number | null>(
    null,
  );
      ...
      <div className="w-full max-w-4xl bg-[#f5f4ef] dark:bg-[#0a0a0a] border-4 border-[#111] p-8 md:p-16 shadow-[16px_16px_0_0_#111] relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <div className="border-4 border-[#111] dark:border-[#f5f4ef] p-6 text-center hover:bg-[#111] hover:text-[#f5f4ef] dark:hover:bg-[#f5f4ef] dark:hover:text-[#111] transition-colors">
            ...
            <div className="font-heading text-5xl font-black text-[#ff3b00]">
              {score}
            </div>
          ...
          <div className="border-4 border-[#111] dark:border-[#f5f4ef] p-6 text-center hover:bg-[#111] hover:text-[#f5f4ef] dark:hover:bg-[#f5f4ef] dark:hover:text-[#111] transition-colors">
            ...
            <div
              className={`font-heading text-5xl font-black ${switches > 0 ? "text-[#ff3b00]" : ""}`}
            >
              {switches}
            </div>
            <div className="text-xs opacity-50 mt-1">
              {switches === 0 ? "perfect focus!" : "threshold breaks"}
            </div>
          </div>
                ...
                <tbody>
                  {leaderboard.map((row, idx) => {
                    const estFocusSeconds = roomDurationMinutes
                      ? Math.max(
                          0,
                          roomDurationMinutes * 60 - row.idleMinutes * 60,
                        )
                      : null;
                    return (
                      <tr
                      ...
                      >
                        <td className="py-2 pr-2 font-heading font-black">
                          #{idx + 1}
                        </td>
                        <td className="py-2 pr-2 font-semibold">
                          {row.userName}
                        </td>
                        <td className="py-2 pr-2 font-heading font-black text-[#ff3b00]">
                          {row.focusScore}
                        </td>
                        <td className="py-2 pr-2">
                          {estFocusSeconds !== null
                            ? fmtSeconds(estFocusSeconds)
                            : "—"}
                        </td>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          ...
          <div className="border-2 border-[#111]/40 dark:border-[#f5f4ef]/30 p-5 text-center bg-white/50 dark:bg-white/5">
            ...
            <div
              className={`font-heading text-3xl font-black ${awayTimeSec > 0 ? "text-[#ff3b00]" : ""}`}
            >
